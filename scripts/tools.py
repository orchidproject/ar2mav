...
def decode_navdata(packet):
    ...
    while True:
        try:
            ...
            break
        if NAVDATA_OPTIONS[option_id] == "CKS":
            print "CHECKSUM"
            break
        elif NAVDATA_OPTIONS[option_id] == "DEMO":
            temp_dict = dict()
            ...
            temp_dict["THETA"] = temp[2]
            temp_dict["PHI"] = temp[3]
            temp_dict["PSI"] = temp[4]
            temp_dict["ALTITUDE"] = temp[5]
            temp_dict["VX"] = temp[6]
            temp_dict["VY"] = temp[7]
            temp_dict["VZ"] = temp[8]
            temp_dict["FRAME_INDEX"] = temp[9]
            data["DEMO"] = temp_dict
            # Skip rest of packet as everything except detection_camera_type is deprecated
        elif NAVDATA_OPTIONS[option_id] == "TIME":
            temp = struct.unpack_from("I", packet, offset)
            data["TIME"] = drone_time_to_milliseconds(temp[0])
        elif NAVDATA_OPTIONS[option_id] == "RAW_MEASURES":
            temp_dict = dict()
            temp = struct.unpack_from("H"*3+"h"*5+"I"+"H"*9+"Iih", packet, offset)
            temp["RAW_ACCS"] = [temp[0], temp[1], temp[2]]
            temp["RAW_GYROS"] = [temp[3], temp[4], temp[5]]
            temp["RAW_GYROS110"] = [temp[6], temp[7]]
            temp["VBAT_RAW"] = temp[8]
            # No idea also
            temp["US_ECHO"] = [temp[9], temp[10], temp[11], temp[12]]
            temp["US_COURBE"] = [temp[13], temp[14], temp[15]]
            temp["ECHO"] = [temp[16], temp[17], temp[18]]
            temp["ALT_TEMP"] = temp[19]
            temp["GRADIENT"] = temp[20]
            data["RAW_MEASURES"] = temp_dict
        elif NAVDATA_OPTIONS[option_id] == "PHYS_MEASURES":
            temp_dict = dict()
            temp = struct.unpack_from("fH"+"f"*6+"I"*3, packet, offset)
            temp["ACCS_TEMP"] = temp[0]
            temp["GYRO_TEMP"] = temp[1]
            temp["PHYS_ACCS"] = [temp[2], temp[3], temp[4]]
            temp["PHYS_GYROS"] = [temp[5], temp[6], temp[7]]
            temp["ALIM3V3"] = temp[8]
            temp["VREF_EPSON"] = temp[9]
            temp["VREF_IDG"] = temp[10]
            data["PHYS_MEASURES"] = temp_dict
        elif NAVDATA_OPTIONS[option_id] == "GYROS_OFFSETS":
            temp = struct.unpack_from("f"*3, packet, offset)
            data["GYROS_OFFSETS"] = [temp[0], temp[1], temp[2]]
        elif NAVDATA_OPTIONS[option_id] == "EULER_ANGLES":
            temp = struct.unpack_from("f"*2, packet, offset)
            data["EULER_ANGLES"] = [temp[0], temp[1]]
        elif NAVDATA_OPTIONS[option_id] == "REFERENCES":
            temp_dict = dict()
        ...
        elif NAVDATA_OPTIONS[option_id] == "TRIMS":
            temp = struct.unpack_from("f"*3, packet, offset)
            data["TRIMS"] = [temp[0], temp[1], temp[2]]
        elif NAVDATA_OPTIONS[option_id] == "RC_REFERENCES":
            temp_dict = dict()
        ...
        elif NAVDATA_OPTIONS[option_id] == "PWM":
            temp_dict = dict()
            ## TODO
            temp = struct.unpack_from("iiiii", packet, offset)
            data["PWM"] = temp_dict
        elif NAVDATA_OPTIONS[option_id] == "ALTITUDE":
            temp_dict = dict()
            ## TODO
            temp = struct.unpack_from("iiiii", packet, offset)
            data["ALTITUDE"] = temp_dict
        elif NAVDATA_OPTIONS[option_id] == "VISION_RAW":
            temp = struct.unpack_from("f"*3, packet, offset)
            data["VISION_RAW"] = [temp[0], temp[1], temp[2]]
        elif NAVDATA_OPTIONS[option_id] == "VISION_OF":
            temp_dict = dict()
        ...
        elif NAVDATA_OPTIONS[option_id] == "VISION":
            temp_dict = dict()
            ## TODO
            temp = struct.unpack_from("iiiii", packet, offset)
            data["VISION"] = temp_dict
        elif NAVDATA_OPTIONS[option_id] == "VISION_PERF":
            temp_dict = dict()
        ...
        elif NAVDATA_OPTIONS[option_id] == "TRACKERS_SEND":
            temp_dict = dict()
        ...
        elif NAVDATA_OPTIONS[option_id] == "VISION_DETECT":
            temp_dict = dict()
            ## TODO
            temp = struct.unpack_from("i", packet, offset)
            data["VISION_DETECT"] = temp_dict
        elif NAVDATA_OPTIONS[option_id] == "WATCHDOG":
            temp = struct.unpack_from("i", packet, offset)
            data["WATCHDOG"] = temp[0]
        elif NAVDATA_OPTIONS[option_id] == "ADC_DATA_FRAME":
            temp_dict = dict()
            temp = struct.unpack_from("I", packet, offset)
        ...
        elif NAVDATA_OPTIONS[option_id] == "VIDEO_STREAM":
            temp_dict = dict()
            ## TODO
            temp = struct.unpack_from("i", packet, offset)
            data["VIDEO_STREAM"] = temp_dict
        elif NAVDATA_OPTIONS[option_id] == "GAMES":
            temp = struct.unpack_from("II", packet, offset)
            data["VIDEO_STREAM"] = [temp[0], temp[1]]
        elif NAVDATA_OPTIONS[option_id] == "PRESSURE_RAW":
            temp = struct.unpack_from("ihii", packet, offset)
            data["PRESSURE_RAW"] = list(temp)
        elif NAVDATA_OPTIONS[option_id] == "MAGNETO":
            temp = struct.unpack_from("ihii", packet, offset)
            ## TODO
            data["MAGNETO"] = list(temp)
        elif NAVDATA_OPTIONS[option_id] == "WIND":
            temp = struct.unpack_from("ihii", packet, offset)
            ## TODO
            data["WIND"] = list(temp)
        elif NAVDATA_OPTIONS[option_id] == "KALMAN_PRESSURE":
            temp = struct.unpack_from("ihii", packet, offset)
            ## TODO
            data["KALMAN_PRESSURE"] = list(temp)
        elif NAVDATA_OPTIONS[option_id] == "HDVIDEO_STREAM":
            temp_dict = dict()
        ...
        elif NAVDATA_OPTIONS[option_id] == "WIFI":
            temp = struct.unpack_from("I", packet, offset)
            data["WIFI"] = temp[0]
        elif NAVDATA_OPTIONS[option_id] == "GPS":
